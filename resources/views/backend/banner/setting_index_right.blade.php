            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form action="{{route('setting.index_insert')}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            @if(empty($data_banner[0]))
                                @for ($i=1; $i <= 3 ; $i++)
                                    <div class="col-md-4 text-center">
                                        <label class="col-form-label ">Banner {{$i}}</label><br>
                                        <!-- <div class="col-md-9 col-sm-9 "> -->
                                        <img src="../images/images.png" class=" img-thumbnail avatar-input{{$i}}"
                                             alt="banner_{{$i}}">
                                        <input class="form-control file-upload file{{$i}} mt-1" type="file"
                                               id="banner_{{$i}}" name="file[]" placeholder=""/>
                                        <div class="row mt-1">
                                        ...
                                        <div class="row mt-1">
                                            <label class="col-form-label col-md-3 text-left">สถานะการแสดง</label>
                                            <div class="col-md-1">
                                                <div class="form-check form-switch mt-2">
                                                    <input class="form-check-input" type="checkbox" id="customSwitch{{$i}}" name="status_show[]" value="1" data-number={{$i}} checked>
                                                    <label class="form-check-label" for="customSwitch{{$i}}" id="{{$i}}">เปิด</label>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @foreach($data_banner as $row)
                                    <div class="col-md-4 text-center">
                                        ...
                                        <img src="{{asset('storage/index/'.$row->file)}}"
                                             class=" img-thumbnail avatar-input{{$row->ordinal}}"
                                             alt="banner_{{$row->ordinal}}">
                                        <input class="form-control file-upload file{{$row->ordinal}} mt-1" type="file"
                                               id="banner_{{$row->ordinal}}" name="file[]" placeholder=""/>
                                        <div class="row m-t-5">
...
{{--                                                           id="customSwitch{{$row->ordinal}}"--}}
{{--                                                           name="status_show[{{$row->ordinal-1}}]" value="1"--}}
{{--                                                           data-number={{$row->ordinal}} @if($row->status_show == 1) checked @endif>--}}
{{--                                                    <label class="custom-control-label"--}}
{{--                                                           for="customSwitch{{$row->ordinal}}" id="{{$row->ordinal}}">--}}
...
{{--                                                </div>--}}
                                                <div class="form-check form-switch mt-2">
                                                    <input class="form-check-input" type="checkbox" id="customSwitch{{$row->ordinal}}" name="status_show[{{$row->ordinal-1}}]" value="1" data-number={{$row->ordinal}} @if($row->status_show == 1) checked @endif>
                                                    <label class="form-check-label" for="customSwitch{{$row->ordinal}}" id="{{$row->ordinal}}">
                                                        @if($row->status_show == 1) เปิด
                                                        @else ปิด
                                                        @endif
                                                    </label>
